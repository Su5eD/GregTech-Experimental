...
public final class CraftingRecipesGen implements ModRecipeProvider {
    ...
    public void buildCraftingRecipes(Consumer<FinishedRecipe> finishedRecipeConsumer) {
        ...
        shaped(Armor.LAPOTRONPACK).define('L', GregTechTags.LARGE_EU_STORE).define('C', ModCoverItem.ENERGY_FLOW_CIRCUIT.getTag()).define('I', GregTechTags.IRIDIUM_ALLOY).define('P', GregTechTags.LAPPACK).define('S', Component.SUPERCONDUCTOR.getTag()).pattern("CLC").pattern("SPS").pattern("CIC").unlockedBy("has_large_eu_store", hasTags(GregTechTags.LARGE_EU_STORE)).save(finishedRecipeConsumer, shapedId("armor/lapotronpack"));
        ...
        shaped(ModBlock.LESUBLOCK).define('M', GregTechTags.CIRCUIT).define('L', Miscellaneous.LAZURITE_CHUNK.getTag()).pattern("LLL").pattern("LML").pattern("LLL").unlockedBy("has_crafting_raw_machine_tier_2", hasTags(GregTechTags.CRAFTING_RAW_MACHINE_TIER_2)).save(finishedRecipeConsumer, shapedId("block/lesublock"));
        ...
        shaped(ModCoverItem.DATA_CONTROL_CIRCUIT, 4).define('E', Component.DATA_STORAGE_CIRCUIT.getTag()).define('I', GregTechTags.IRIDIUM_ALLOY).define('A', GregTechTags.ADVANCED_CIRCUIT).pattern("AEA").pattern("EIE").pattern("AEA").unlockedBy("has_iridium_alloy", hasTags(GregTechTags.IRIDIUM_ALLOY)).save(finishedRecipeConsumer, shapedId("component/data_control_circuit"));
        ...
        shaped(Upgrade.QUANTUM_CHEST).define('A', Component.DATA_ORB.getTag()).define('S', ModCoverItem.SCREEN).define('D', Plate.ALUMINIUM.getTag()).define('T', GregTechTags.TELEPORTER).define('B', GregTechTags.CRAFTING_RAW_MACHINE_TIER_4).pattern("ASA").pattern("BTB").pattern("ADA").unlockedBy("has_data_orb", hasTags(Component.DATA_ORB.getTag())).save(finishedRecipeConsumer, shapedId("component/quantum_chest_upgrade"));
...
//        shaped(ModCoverItem.SOLAR_PANEL_LV).define('S', GregTechTags.CRAFTING_SOLAR_PANEL).define('T', GregTechTags.ADVANCED_CIRCUIT).pattern("SSS").pattern("STS").pattern("SSS").unlockedBy("has_solar_panel", has(GregTechTags.CRAFTING_SOLAR_PANEL)).save(finishedRecipeConsumer, id("component/solar_panel_lv"));
        shaped(Component.SUPERCONDUCTOR, 4).define('L', ModCoverItem.ENERGY_FLOW_CIRCUIT.getTag()).define('W', Plate.TUNGSTEN.getTag()).define('I', GregTechTags.IRIDIUM_ALLOY).define('C', tagsIngredient(NuclearCoolantPack.HELIUM_60K.getTag(), NuclearCoolantPack.NAK_60K.getTag())).pattern("CCC").pattern("WIW").pattern("LLL").unlockedBy("has_energy_flow_circuit", hasTags(ModCoverItem.ENERGY_FLOW_CIRCUIT.getTag())).save(finishedRecipeConsumer, shapedId("component/superconductor"));
        shaped(Component.SUPERCONDUCTOR, 4).define('L', ModCoverItem.ENERGY_FLOW_CIRCUIT.getTag()).define('W', Plate.TUNGSTEN.getTag()).define('I', GregTechTags.IRIDIUM_ALLOY).define('C', GregTechTags.SEXTUPLE_REACTOR_COOLANT_CELL).pattern("CCC").pattern("WIW").pattern("LLL").unlockedBy("has_energy_flow_circuit", hasTags(ModCoverItem.ENERGY_FLOW_CIRCUIT.getTag())).save(finishedRecipeConsumer, shapedId("component/superconductor_2"));
        ...
        // Materials
        shapeless(Dust.INVAR, 3).requires(Ingredient.of(Dust.IRON.getTag()), 2).requires(Dust.NICKEL).unlockedBy("has_nickel_dust", has(Dust.NICKEL)).save(finishedRecipeConsumer, shapelessId("material/invar_dust"));
        ...
        shapeless(Smalldust.LEAD, 3).requires(Dust.IRON.getTag()).requires(Smalldust.NICKEL.getTag()).unlockedBy("has_small_dusts_nickel", hasTags(Smalldust.NICKEL.getTag())).save(finishedRecipeConsumer, shapelessId("material/lead_smalldust"));
//        conditionalShapeless(Dust.STEEL).requires(SelectedProfileCondition.REFINED_IRON, tagsIngredient(Dust.IRON.getTag(), GregTechTags.dust("refined_iron"))).requires(SelectedProfileCondition.REGULAR_IRON, Smalldust.IRON.getTag()).requires(Dust.COAL.getTag(), 2).unlockedBy("has_coal_dust", has(Dust.COAL.getTag())).save(finishedRecipeConsumer, id("material/steel_dust"));
        ...
        shapeless(Dust.SALTPETER, 10).requires(VanillaFluidIngredient.ofFluid(ModFluid.POTASSIUM.getTag()), 2).requires(VanillaFluidIngredient.ofFluid(ModFluid.NITROGEN.getTag()), 2).requires(VanillaFluidIngredient.ofFluid(GregTechTags.AIR), 3).unlockedBy("has_potassium_fluid", hasFluid(ModFluid.POTASSIUM.getTag())).save(finishedRecipeConsumer, shapelessId("material/saltpeter_dust"));
        ...
        // Nuclear
        shaped(NuclearCoolantPack.HELIUM_60K).define('H', VanillaFluidIngredient.ofFluid(ModFluid.HELIUM.getTag())).define('T', Plate.TIN.getTag()).pattern(" T ").pattern("THT").pattern(" T ").unlockedBy("has_tin_plate", hasTags(Plate.TIN.getTag())).save(finishedRecipeConsumer, shapedId("nuclear/helium_60k_coolant"));
        ...
        shaped(NuclearCoolantPack.NAK_60K).define('N', VanillaFluidIngredient.ofFluid(ModFluid.SODIUM.getTag())).define('K', VanillaFluidIngredient.ofFluid(ModFluid.POTASSIUM.getTag())).define('T', Plate.TIN.getTag()).define('C', GregTechTags.REACTOR_COOLANT_CELL).pattern("TNT").pattern("KCK").pattern("TNT").unlockedBy("has_reactor_coolant_cell", hasTags(GregTechTags.REACTOR_COOLANT_CELL)).save(finishedRecipeConsumer, shapedId("nuclear/nak_60k_coolant"));
        shaped(NuclearCoolantPack.NAK_60K).define('N', VanillaFluidIngredient.ofFluid(ModFluid.SODIUM.getTag())).define('K', VanillaFluidIngredient.ofFluid(ModFluid.POTASSIUM.getTag())).define('T', Plate.TIN.getTag()).define('C', GregTechTags.REACTOR_COOLANT_CELL).pattern("TKT").pattern("NCN").pattern("TKT").unlockedBy("has_reactor_coolant_cell", hasTags(GregTechTags.REACTOR_COOLANT_CELL)).save(finishedRecipeConsumer, shapedId("nuclear/nak_60k_coolant_vertical"));
        ...
        shaped(Tool.TESLA_STAFF).define('L', GregTechTags.LARGE_EU_STORE).define('S', Component.SUPERCONDUCTOR.getTag()).define('I', GregTechTags.IRIDIUM_ALLOY).pattern("LS ").pattern("SI ").pattern("  I").unlockedBy("has_iridium_alloy", hasTags(GregTechTags.IRIDIUM_ALLOY)).save(finishedRecipeConsumer, shapedId("tool/tesla_staff"));
