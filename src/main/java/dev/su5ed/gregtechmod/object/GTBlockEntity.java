...
public enum GTBlockEntity implements BlockItemProvider, BlockEntityProvider {
    AUTO_MACERATOR(AutomaticMaceratorBlockEntity::new);

    private final BlockEntityType<?> type;
    private final Lazy<Block> block;
    private final Lazy<Item> item;

    GTBlockEntity(BlockEntityType.BlockEntitySupplier<? extends BaseBlockEntity> factory) {
        String name = getName();
        this.block = Lazy.of(() -> new BaseEntityBlock(factory).setRegistryName(name));
