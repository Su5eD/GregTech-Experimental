...
public class CoverConveyor extends CoverInventory {
    ...
    public void doCoverThings() {
        ...
        if (te instanceof IElectricalMachine && mode.consumesEnergy(side) && ((IElectricalMachine)te).getUniversalEnergyCapacity() >= 128) {
            if (((IElectricalMachine) te).getUniversalEnergy() >= 128) ((IElectricalMachine)te).useEnergy(moveItemStack((TileEntity)te, side, mode), false);
