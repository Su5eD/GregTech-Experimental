...
@ZenClass("mods.gregtechmod.fuel.Diesel")
@ZenRegister
public class DieselFuels {

    @ZenMethod
    public static void addFuel(IIngredient input, double energy, @Optional IItemStack output) {
        CraftTweakerFuels.addSimpleFuel(GtFuels.diesel, input, energy, output);
    }

    @ZenMethod
    public static void removeFuel(ILiquidStack fluid) {
        GtFuels.diesel.removeFuel(CraftTweakerMC.getFluid(fluid.getDefinition()));
    }

    @ZenMethod
    public static void removeFuel(IItemStack stack) {
        GtFuels.diesel.removeFuel(CraftTweakerMC.getItemStack(stack));
    }
}
