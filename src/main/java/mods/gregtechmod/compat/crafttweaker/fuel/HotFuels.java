...
@ZenClass("mods.gregtechmod.fuel.Hot")
@ZenRegister
public class HotFuels {

    @ZenMethod
    public static void addFuel(IIngredient input, double energy, @Optional IItemStack[] output) {
        CraftTweakerFuels.addMultiFuel(GtFuels.hot, input, energy, output);
    }

    @ZenMethod
    public static void removeFuel(ILiquidStack fluid) {
        GtFuels.hot.removeFuel(CraftTweakerMC.getFluid(fluid.getDefinition()));
    }

    @ZenMethod
    public static void removeFuel(IItemStack stack) {
        GtFuels.hot.removeFuel(CraftTweakerMC.getItemStack(stack));
    }
}
