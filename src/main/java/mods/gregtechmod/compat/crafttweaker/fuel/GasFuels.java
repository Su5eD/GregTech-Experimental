...
@ZenClass("mods.gregtechmod.fuel.Gas")
@ZenRegister
public class GasFuels {

    @ZenMethod
    public static void addFuel(IIngredient input, double energy, @Optional IItemStack output) {
        CraftTweakerFuels.addSimpleFuel(GtFuels.gas, input, energy, output);
    }

    @ZenMethod
    public static void removeFuel(ILiquidStack fluid) {
        GtFuels.gas.removeFuel(CraftTweakerMC.getFluid(fluid.getDefinition()));
    }

    @ZenMethod
    public static void removeFuel(IItemStack stack) {
        GtFuels.gas.removeFuel(CraftTweakerMC.getItemStack(stack));
    }
}
