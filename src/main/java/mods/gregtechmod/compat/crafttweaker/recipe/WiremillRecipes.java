...
public class WiremillRecipes {
    ...
    public static void addRecipe(IIngredient input, IItemStack output, int duration, double energyCost) {
        ...
        IMachineRecipe<IRecipeIngredient, List<ItemStack>> recipe = GregTechAPI.getRecipeFactory().makeWiremillRecipe(ingredient, outputStack, duration, energyCost);
        CraftTweakerAPI.apply(new AddRecipeAction<>(GtRecipes.wiremill, recipe));
    }

    @ZenMethod
    public static void removeRecipe(IItemStack input) {
        CraftTweakerAPI.apply(new RemoveRecipeAction<>(GtRecipes.wiremill, GtRecipes.wiremill.getRecipeFor(CraftTweakerMC.getItemStack(input))));
    }

    @ZenMethod
    public static void removeByOutput(IItemStack output) {
        ItemStack stack = CraftTweakerMC.getItemStack(output);
        CraftTweakerAPI.apply(new RemoveRecipeByOutputAction<>(GtRecipes.wiremill, stacks -> stacks.stream().anyMatch(stack::isItemEqual)));
    }
}
