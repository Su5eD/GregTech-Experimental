...
public class ItemChainsawAdvanced extends ItemElectricTool implements IHasModel {

    public ItemChainsawAdvanced() {
        super(null, 800, HarvestLevel.Iron, EnumSet.of(ToolClass.Axe, ToolClass.Sword, ToolClass.Shears));
        setRegistryName("chainsaw_advanced");
        this.maxCharge = 300000;
        this.transferLimit = 1000;
        this.tier = 3;
        this.efficiency = 24.0F;
        ItemInit.ITEMS.put("chainsaw_advanced", this);
    }

    @Override
    public String getTranslationKey() {
        return "item.chainsaw_advanced";
    }
    ...
    public void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        ...
        if (itemstack != null && itemstack.getItem() == this && entity instanceof IShearable &&
                ...
                .use(itemstack, this.operationEnergyCost, player)) {
    ...
    public boolean onBlockStartBreak(ItemStack itemstack, BlockPos pos, EntityPlayer player) {
        ...
        if (block instanceof IShearable) {
            ...
            if (target.isShearable(itemstack, world, pos) && ElectricItem.manager
                    .use(itemstack, this.operationEnergyCost, player)) {
                List<ItemStack> drops = target.onSheared(itemstack, world, pos,
                        EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, itemstack));
                for (ItemStack stack : drops)
