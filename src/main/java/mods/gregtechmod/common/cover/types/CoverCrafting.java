...
public class CoverCrafting extends CoverGeneric {
    ...
    public boolean onCoverRightClick(EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (player instanceof EntityPlayerMP) {
            ...
            player.openContainer = new ContainerWorkbench(player.inventory, ((TileEntity)te).getWorld(), ((TileEntity)te).getPos()) {
                @Override
                public boolean canInteractWith(EntityPlayer playerIn) {
                    System.out.println(((TileEntity)te).getWorld().getBlockState(((TileEntity)te).getPos()).getBlock().equals(((TileEntity)te).getBlockType()));
