package mods.gregtechmod.common.recipe;
...
public class RecipeLoader {
    private static Queue<Runnable> pendingRecipes = new ArrayDeque<>();
    ...
    static void loadRecipes() {
        ...
        try {
            ...
            GregtechMod.log.warn(LogCategory.Recipe, e, "Failed to load recipe.");
        }
    ...
    private static void loadMachineRecipes(Config config, IMachineRecipeManager<IRecipeInput, Collection<ItemStack>, ?> machine, MachineType type) {
        ...
        GregtechMod.log.log(LogCategory.Recipe, Level.INFO, "Successfully loaded " + successful + " from " + amount + " recipes for " + config.name);
    }
    ...
    private static boolean loadMachineRecipe(Config.Value value, IMachineRecipeManager<IRecipeInput, Collection<ItemStack>, ?> machine, MachineType type, boolean lastAttempt) {
        ...
        if (input == null) {
            if (lastAttempt) {
                GregtechMod.log.warn(LogCategory.Recipe, new Config.ParseException("invalid input: " + value.name, value), "Skipping recipe due to unresolvable input %s.", value.name);
            } else {
        ...
        try {
            for (String part : splitWhitespace(value.getString())) {
                ...
                if (cOutput == null) {
                    if (lastAttempt) {
                        GregtechMod.log.warn(LogCategory.Recipe, new Config.ParseException("invalid output specified: " + part, value), "Skipping recipe using %s due to unresolvable output %s.", new Object[] { value.name, part });
                    } else {
                        pendingRecipes.add(() -> loadMachineRecipe(value, machine, type, true));
                    }
                    return false;
            ...
            if (!type.tagsRequired.isEmpty() && (metadata.isEmpty() || !type.hasRequiredTags(metadata))) {
                GregtechMod.log.warn(LogCategory.Recipe, "Could not add machine recipe: " + value.name + " missing tag.");
                return false;
            }
            if (metadata.isEmpty())
                metadata = null;
    ...
    private static List<String> splitWhitespace(String text) {
        try {
            return (List<String>)WHITESPACE.invoke(null, new Object[] { text });
        } catch (Exception e) {
